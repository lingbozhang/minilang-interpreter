...
int main() {
  ...
  for (;;) {
    ...
    // Help
      ...
      std::cout << "To use this interactive REPL, just type in regular "
                   "MiniLang commands and hit\n";
      std::cout
          << "enter. You can also make use of the following commands: \n\n";

      std::cout << " #load \e[1;3;4mfile-path\e[0m  ";
      std::cout
          << "Loads variable and function declarations from a specified\n";
      std::cout << std::setw(18);
      std::cout << ""
                << "file into memory, e.g.\n";
      std::cout << std::setw(18);
      std::cout << ""
                << ">>> #load ~/hello_world.prog\n\n";
      std::cout << " #quit            Exits the MiniLang REPL.\n\n";
      std::cout << " #st              Displays the symbol table, a list of "
                   "currently declared \n";
      std::cout << std::setw(18);
      std::cout << ""
                << "functions and variables in the global scope.\n\n";
    ...
    // Load File
      ...
      if (input_line.size() <= 6) {
        ...
        if (!file)
          std::cout << "Could not load file from \"" + file_dir + "\"."
                    << std::endl;

        else {
          // Convert whole program to std::string
          std::string line;
          while (std::getline(file, line)) program.append(line + "\n");
    ...
    // Symbol Table
      ...
      std::cout << "\e[1mCurrently Declared Variables and Functions\e[0m"
                << std::endl;
      TextTable vt('-', '|', '+');
      vt.add("Name");
      vt.add("Type");
    ...
    try {
      ...
      // Catch by trying to parse as expression
        ...
        try {
          ...
          parser =
              parser::Parser(&expr_lexer, 0);  // do not consume first token
      ...
      if (file_load) std::cout << "File loaded successfully." << std::endl;
