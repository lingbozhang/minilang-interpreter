...
Lexer::Lexer(std::string &program) {
  ...
  while (current_index <= program.length()) {
    ...
    if (t.type != TOK_COMMENT) tokens.push_back(t);
  }
}
...
int Lexer::transition_delta(int s, char sigma) {
  ...
  switch (sigma) {
    ...
    default:
      ...
      // Else if Printable
      if ((0x20 <= ascii) && (ascii <= 0x7E)) return transitions[PRINTABLE][s];
...
Token Lexer::next_token(std::string &program, unsigned int &current_index) {
  ...
  if (current_state == -1) throw std::runtime_error("Lexical error.");

  if (is_final[current_state])
    return Token(current_state, std::move(lexeme),
                 get_line_number(program, current_index));
  else
    throw std::runtime_error(
        "Lexical error on line " +
        std::to_string(get_line_number(program, current_index)) + ".");
}
