...
ASTFunctionDefinitionNode::ASTFunctionDefinitionNode(
    ...
    std::vector<std::pair<std::string, TYPE>> parameters, TYPE type,
    ASTBlockNode *block, unsigned int line_number)
    : identifier(std::move(identifier)),
...
// Expression Nodes
ASTBinaryExprNode::ASTBinaryExprNode(std::string op, ASTExprNode *left,
                                     ASTExprNode *right,
                                     unsigned int line_number)
    : op(std::move(op)), left(left), right(right), line_number(line_number) {}

ASTIdentifierNode::ASTIdentifierNode(std::string identifier,
                                     unsigned int line_number)
    : identifier(std::move(identifier)), line_number(line_number) {}

ASTUnaryExprNode::ASTUnaryExprNode(std::string unary_op, ASTExprNode *expr,
                                   unsigned int line_number)
    : unary_op(std::move(unary_op)), expr(expr), line_number(line_number) {}
