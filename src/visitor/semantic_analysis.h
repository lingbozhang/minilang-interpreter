...
class SemanticScope {
 public:
  ...
  void declare(std::string, parser::TYPE, std::vector<parser::TYPE>,
               unsigned int);
 ...
 private:
  std::map<std::string, std::pair<parser::TYPE, unsigned int>>
      variable_symbol_table;

  std::multimap<std::string, std::tuple<parser::TYPE, std::vector<parser::TYPE>,
                                        unsigned int>>
