...
bool SemanticScope::already_declared(std::string identifier,
  ...
  if (std::distance(funcs.first, funcs.second) == 0) return false;

  // Check signature for each function in multimap
  for (auto i = funcs.first; i != funcs.second; i++)
    if (std::get<1>(i->second) == signature) return true;

  // Function with matching signature not found
  return false;
}

void SemanticScope::declare(std::string identifier, parser::TYPE type,
...
void SemanticScope::declare(std::string identifier, parser::TYPE type,
                            std::vector<parser::TYPE> signature,
                            unsigned int line_number) {
  function_symbol_table.insert(std::make_pair(
      identifier, std::make_tuple(type, signature, line_number)));
}
...
parser::TYPE SemanticScope::type(std::string identifier) {
  ...
  throw std::runtime_error(
      "Something went wrong when determining the type of '" + identifier +
      "'.");
}
...
parser::TYPE SemanticScope::type(std::string identifier,
  ...
  for (auto i = funcs.first; i != funcs.second; i++)
    if (std::get<1>(i->second) == signature) return std::get<0>(i->second);
...
unsigned int SemanticScope::declaration_line(std::string identifier) {
  ...
  throw std::runtime_error(
      "Something went wrong when determining the line number of '" +
      identifier + "'.");
}

unsigned int SemanticScope::declaration_line(
    std::string identifier, std::vector<parser::TYPE> signature) {
  auto funcs = function_symbol_table.equal_range(identifier);
  ...
  // Check signature for each
  for (auto i = funcs.first; i != funcs.second; i++)
    if (std::get<1>(i->second) == signature) return std::get<2>(i->second);
...
SemanticScope::function_list() {
  std::vector<std::pair<std::string, std::string>> list;

  for (auto func = function_symbol_table.begin(),
            last = function_symbol_table.end();
    ...
    list.emplace_back(
        std::make_pair(func_name, type_str(std::get<0>(func->second))));
  }
...
void SemanticAnalyser::visit(parser::ASTProgramNode *prog) {
  ...
  for (auto &statement : prog->statements) statement->accept(this);
}
...
void SemanticAnalyser::visit(parser::ASTDeclarationNode *decl) {
  ...
  if (current_scope->already_declared(decl->identifier))
    throw std::runtime_error(
        "Variable redeclaration on line " + std::to_string(decl->line_number) +
        ". '" + decl->identifier +
        "' was already declared in this scope on line " +
  ...
  // types don't match
    ...
    throw std::runtime_error("Found " + type_str(current_expression_type) +
                             " on line " + std::to_string(decl->line_number) +
                             " in definition of '" + decl->identifier +
...
void SemanticAnalyser::visit(parser::ASTAssignmentNode *assign) {
  ...
  unsigned long i;
  for (i = scopes.size() - 1; !scopes[i]->already_declared(assign->identifier);
       i--)
    if (i <= 0)
      throw std::runtime_error(
          "Identifier '" + assign->identifier + "' being reassigned on line " +
  ...
  // otherwise throw error
    ...
    throw std::runtime_error(
        "Mismatched type for '" + assign->identifier + "' on line " +
...
void SemanticAnalyser::visit(parser::ASTReturnNode *ret) {
  ...
  if (!functions.empty() && current_expression_type != functions.top())
    throw std::runtime_error("Invalid return type on line " +
                             std::to_string(ret->line_number) + ". Expected " +
                             type_str(functions.top()) + ", found " +
...
void SemanticAnalyser::visit(parser::ASTIfNode *ifnode) {
  ...
  if (current_expression_type != parser::BOOL)
    throw std::runtime_error("Invalid if-condition on line " +
                             std::to_string(ifnode->line_number) +
  ...
  if (ifnode->else_block) ifnode->else_block->accept(this);
}
...
void SemanticAnalyser::visit(parser::ASTWhileNode *whilenode) {
  ...
  // Make sure it is boolean
  if (current_expression_type != parser::BOOL)
    throw std::runtime_error("Invalid while-condition on line " +
                             std::to_string(whilenode->line_number) +
                             ", expected boolean expression.");
...
void SemanticAnalyser::visit(parser::ASTFunctionDefinitionNode *func) {
  ...
  for (auto &scope : scopes)
    if (scope->already_declared(func->identifier, func->signature)) {
      ...
      throw std::runtime_error(
          "Error on line " + std::to_string(func->line_number) + ". Function " +
          func->identifier + signature + " already defined on line " +
          std::to_string(line) + ".");
    }

  // Add function to symbol table
  scopes.back()->declare(func->identifier, func->type, func->signature,
                         func->line_number);
  ...
  if (!returns(func->block))
    throw std::runtime_error("Function " + func->identifier +
                             " defined on line " +
                             std::to_string(func->line_number) +
                             " is not guaranteed to " + "return a value.");
...
void SemanticAnalyser::visit(parser::ASTBinaryExprNode *bin) {
  ...
  // + works for all types except bool
    ...
    if (l_type == parser::BOOL || r_type == parser::BOOL)
      throw std::runtime_error(
          "Invalid operand for '+' operator, expected numerical or string"
          " operand on line " +
          std::to_string(bin->line_number) + ".");
    ...
    // only one is string, error
      ...
      throw std::runtime_error("Mismatched operands for '+' operator, found " +
                               type_str(l_type) + " on the left, but " +
                               type_str(r_type) + " on the right (line " +
                               std::to_string(bin->line_number) + ").");
  ...
  // and/or only work for bool
    ...
    if (l_type == parser::BOOL && r_type == parser::BOOL)
      ...
      throw std::runtime_error("Expected two boolean-type operands for '" + op +
                               "' operator " + "on line " +
                               std::to_string(bin->line_number) + ".");
  }
  ...
  // rel-ops only work for numeric types
    ...
    if ((l_type != parser::REAL && l_type != parser::INT) ||
      ...
      throw std::runtime_error("Expected two numerical operands for '" + op +
                               "' operator " + "on line " +
                               std::to_string(bin->line_number) + ".");
    current_expression_type = parser::BOOL;
  }
  ...
  // == and != only work for like types
    ...
    if (l_type != r_type && (l_type != parser::REAL || r_type != parser::INT) &&
      ...
      throw std::runtime_error("Expected arguments of the same type '" + op +
                               "' operator " + "on line " +
                               std::to_string(bin->line_number) + ".");
    current_expression_type = parser::BOOL;
  }
...
void SemanticAnalyser::visit(parser::ASTIdentifierNode *id) {
  ...
  for (i = scopes.size() - 1; !scopes[i]->already_declared(id->identifier); i--)
    if (i <= 0)
      throw std::runtime_error(
          "Identifier '" + id->identifier + "' appearing on line " +
  ...
  switch (current_expression_type) {
    ...
    case parser::REAL:
      if (un->unary_op != "+" && un->unary_op != "-")
        throw std::runtime_error(
            "Operator '" + un->unary_op + "' in front of numerical " +
            "expression on line " + std::to_string(un->line_number) + ".");
      break;
    case parser::BOOL:
      if (un->unary_op != "not")
        throw std::runtime_error(
            "Operator '" + un->unary_op + "' in front of boolean " +
            "expression on line " + std::to_string(un->line_number) + ".");
      break;
    default:
      throw std::runtime_error("Incompatible unary operator '" + un->unary_op +
                               "' in front of " + "expression on line " +
                               std::to_string(un->line_number) + ".");
  }
}
...
void SemanticAnalyser::visit(parser::ASTFunctionCallNode *func) {
  ...
  unsigned long i;
  for (i = scopes.size() - 1;
       !scopes[i]->already_declared(func->identifier, signature); i--)
    if (i <= 0) {
      ...
      func_name += ")";
      throw std::runtime_error(
          "Function '" + func_name + "' appearing on line " +
  ...
  current_expression_type =
      scopes[i]->type(func->identifier, std::move(signature));
}
...
bool SemanticAnalyser::returns(parser::ASTStatementNode *stmt) {
  // Base case: if the statement is a return statement, then it definitely
  // returns
  if (dynamic_cast<parser::ASTReturnNode *>(stmt)) return true;
  ...
  if (auto block = dynamic_cast<parser::ASTBlockNode *>(stmt))
    for (auto &blk_stmt : block->statements)
      if (returns(blk_stmt)) return true;

  // An if-(else) block returns only if both the if and the else statement
  // return.
