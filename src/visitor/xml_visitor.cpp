...
std::string XMLVisitor::indentation() {
  ...
  for (unsigned int i = 0; i < indentation_level; i++) tabs += TAB;
...
void XMLVisitor::visit(parser::ASTProgramNode *program) {
  ...
  for (auto &statement : program->statements) statement->accept(this);
...
void XMLVisitor::visit(parser::ASTAssignmentNode *assign) {
  ...
  // Add identifier
  xmlfile << indentation() << "<id>" << assign->identifier << "</id>"
          << std::endl;
...
void XMLVisitor::visit(parser::ASTBlockNode *block) {
  ...
  // For each statement, accept
  for (auto &statement : block->statements) statement->accept(this);
...
void XMLVisitor::visit(parser::ASTFunctionDefinitionNode *func) {
  ...
  xmlfile << indentation()
          << "<func-def type = \"" + type_str(func->type) + "\">" << std::endl;
  ...
  for (auto &param : func->parameters) {
    xmlfile << indentation()
            << "<param type = \"" + type_str(param.second) + "\">" +
                   param.first + "</param>"
            << std::endl;
  }
  ...
  switch (t) {
    ...
    default:
      throw std::runtime_error(
          "Invalid type encountered in syntax tree when generating XML.");
  }
}
...
XMLVisitor::~XMLVisitor() { xmlfile.close(); }