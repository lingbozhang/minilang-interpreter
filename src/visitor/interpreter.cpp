...
bool InterpreterScope::already_declared(std::string identifier,
  ...
  for (auto i = funcs.first; i != funcs.second; i++)
    if (std::get<0>(i->second) == signature) return true;
...
void InterpreterScope::declare(std::string identifier,
...
void InterpreterScope::declare(std::string identifier,
  ...
  function_symbol_table.insert(std::make_pair(
      identifier, std::make_tuple(signature, variable_names, block)));
}
...
std::vector<std::string> InterpreterScope::variable_names_of(
    std::string identifier, std::vector<parser::TYPE> signature) {
  auto funcs = function_symbol_table.equal_range(identifier);

  // Match given signature to function in multimap
  for (auto i = funcs.first; i != funcs.second; i++)
    if (std::get<0>(i->second) == signature) return std::get<1>(i->second);
}

parser::ASTBlockNode *InterpreterScope::block_of(
    std::string identifier, std::vector<parser::TYPE> signature) {
  auto funcs = function_symbol_table.equal_range(identifier);
...
InterpreterScope::variable_list() {
  std::vector<std::tuple<std::string, std::string, std::string>> list;

  for (auto const &var : variable_symbol_table) switch (var.second.first) {
      case parser::INT:
        list.emplace_back(std::make_tuple(var.first, "int",
                                          std::to_string(var.second.second.i)));
        break;
      case parser::REAL:
        list.emplace_back(std::make_tuple(var.first, "real",
                                          std::to_string(var.second.second.r)));
        break;
      case parser::BOOL:
      ...
      case parser::STRING:
        list.emplace_back(
            std::make_tuple(var.first, "string", var.second.second.s));
        break;
    }
  ...
  switch (decl->type) {
    ...
    case parser::REAL:
      if (current_expression_type == parser::INT)
        ...
        scopes.back()->declare(decl->identifier, current_expression_value.r);
      break;
    case parser::BOOL:
      scopes.back()->declare(decl->identifier, current_expression_value.b);
      break;
    case parser::STRING:
      scopes.back()->declare(decl->identifier, current_expression_value.s);
      break;
  }
}
  ...
  switch (scopes[i]->type_of(assign->identifier)) {
    ...
    case parser::REAL:
      if (current_expression_type == parser::INT)
        ...
        scopes[i]->declare(assign->identifier, current_expression_value.r);
      break;
    case parser::BOOL:
      scopes[i]->declare(assign->identifier, current_expression_value.b);
      break;
    case parser::STRING:
      scopes[i]->declare(assign->identifier, current_expression_value.s);
      break;
  }
}
...
void visitor::Interpreter::visit(parser::ASTBinaryExprNode *bin) {
  ...
  if (op == "+" || op == "-" || op == "*" || op == "/") {
    ...
    // At least one real
      ...
      if (l_type == parser::INT) l = l_value.i;
      if (r_type == parser::INT) r = r_value.i;
      if (op == "+")
        v.r = l + r;
      else if (op == "-")
        v.r = l - r;
...
void visitor::Interpreter::visit(parser::ASTUnaryExprNode *un) {
  ...
  switch (current_expression_type) {
    case parser::INT:
      if (un->unary_op == "-") current_expression_value.i *= -1;
      break;
    case parser::REAL:
      if (un->unary_op == "-") current_expression_value.r *= -1;
      break;
    case parser::BOOL:
...
void visitor::Interpreter::visit(parser::ASTFunctionCallNode *func) {
  ...
  for (auto param : func->parameters) {
    ...
    current_function_arguments.emplace_back(current_expression_type,
  ...
  current_function_parameters =
      scopes[i]->variable_names_of(func->identifier, signature);
...
std::pair<parser::TYPE, value_t> Interpreter::current_expr() {
  return std::move(
      std::make_pair(current_expression_type, current_expression_value));
};
